...
@implementation UIImage (TKFXBlurView)
...
@interface TKFXBlurScheduler : NSObject
...
@interface TKFXBlurLayer: CALayer
...
@implementation TKFXBlurScheduler

+ (instancetype)sharedInstance
{
    static TKFXBlurScheduler *sharedInstance = nil;
    if (!sharedInstance)
    {
        sharedInstance = [[TKFXBlurScheduler alloc] init];
    }
...
- (void)setBlurEnabled:(BOOL)blurEnabled
{
    ...
    if (blurEnabled)
    {
        for (TKFXBlurView *view in self.views)
        {
...
- (void)addView:(TKFXBlurView *)view
{
...
- (void)removeView:(TKFXBlurView *)view
{
...
- (void)updateAsynchronously
{
    if (self.blurEnabled && !self.updating && self.updatesEnabled > 0 && [self.views count])
    {
        ...
        {
            TKFXBlurView *view = self.views[i];
...
@implementation TKFXBlurView

+ (void)setBlurEnabled:(BOOL)blurEnabled
{
    [TKFXBlurScheduler sharedInstance].blurEnabled = blurEnabled;
}

+ (void)setUpdatesEnabled
{
    [[TKFXBlurScheduler sharedInstance] setUpdatesEnabled];
}

+ (void)setUpdatesDisabled
{
    [[TKFXBlurScheduler sharedInstance] setUpdatesDisabled];
}

+ (Class)layerClass
{
    return [TKFXBlurLayer class];
}
...
- (TKFXBlurLayer *)blurLayer
{
    return (TKFXBlurLayer *)self.layer;
}

- (TKFXBlurLayer *)blurPresentationLayer
{
    TKFXBlurLayer *blurLayer = [self blurLayer];
    return (TKFXBlurLayer *)blurLayer.presentationLayer ?: blurLayer;
}
...
- (void)schedule
{
    ...
    {
        [[TKFXBlurScheduler sharedInstance] addView:self];
    }
    else
    {
        [[TKFXBlurScheduler sharedInstance] removeView:self];
    }
}
...
- (BOOL)shouldUpdate
{
    ...
    self.blurEnabled && [TKFXBlurScheduler sharedInstance].blurEnabled &&
...
- (UIImage *)snapshotOfUnderlyingView
{
    __strong TKFXBlurLayer *blurLayer = [self blurPresentationLayer];
    __strong CALayer *underlyingLayer = [self underlyingLayer];
